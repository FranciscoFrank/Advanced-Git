...
>[!NOTE]
>
>``` Git Hooks ``` - потужний інструмент який дозволяє оптимізувати та автоматизувати процес розробки. Функціонал хукі дуже обширний завдяки ним можна робити наступне:
>\
>1. Перевіряти синтаксичні помилки в коді
>\
>2. Форматувати код для забезпечення єдиного стиля форматування
>\
>3. Запуск тестів для перевірки коду на роботоздатність
>\
>4. Перевірка тексту коміта на відповідність правилам
>\
>5. Перевірка якості коду та відповідності правилам стилізації (eslint (JavaScript) / stylelint (CSS))
>\
>6. Перевірка залежностей (npm (JavaScript) / bandit (Python))
>\
>7. Автоматичне розгортання проекту на сервері після отримання змін
