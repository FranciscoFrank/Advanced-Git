...
>[!NOTE]
>
>Функціонал, який пропонує ``` Git Submodule ```, корисний, коли йде розробка сайту, і команда розробки не хоче писати код, а використати вже готову бібліотеку для цього (Наприклад, сайт використовує Atom feeds; в такому разі було б краще використати вже готову бібліотеку). Звичайно, можна використати готовий код цієї бібліотеки прямо в своєму проєкті, але вона може не працювати на пристроях всіх клієнтів, а в разі оновлення цієї бібліотеки вам буде досить важко злиття цих змін із вашим проєктом. Саме для таких ситуацій і придумали ``` Git Submodule ```.
>
>У разі, коли нам потрібна якась бібліотека, ми можемо встановити її репозиторій всередині репозиторія нашого проєкту. Для цього знадобиться команда: ``` git submodule add <URL-адреса репозиторія з необхідною нам бібліотекою (або іншим проєктом)> ```. Тепер цей репозиторій встановлений у нас як окрема директорія. Разом із цим створився файл ``` .gitmodules ```, у якому міститься інформація про назву модуля та URL-шлях до репозиторія. У разі, якщо нам буде потрібен ще один підмодуль, аналогічна інформація з'явиться в цьому ж файлі.
>\
>Варто зауважити, що, незважаючи на те, що модуль є окремою директорією, в основній директорії Git розглядає її як підмодуль і не слідкує за її вмістом. Тому вміст цієї директорії — це останній коміт із того репозиторія.
>
>Розглянемо випадок, коли ми клонували проєкт із модулями. Після клонування, якщо ми захочемо побачити вміст директорії з модулем, вона буде пустою. Для ініціалізації сабмодулів потрібно прописати ``` git submodule init ```, або, якщо ми не хочемо зайвий раз писати команду, можна передати прапорець ``` --recurse-submodules ``` до команди ``` git clone ```, і вона автоматично зробить ініціалізацію та оновить кожен підмодуль у репозиторії.
>
>Уявімо, що нам потрібно переглянути та застосувати зміни в репозиторії встановленого модуля. Для цього переходимо в його директорію і прописуємо ``` git fetch ```, а потім ``` git merge ``` або обійтися одним ``` git pull ```. А якщо сабмодулів багато, можна прописати ``` git submodule update --remote ``` - Git перейде до ваших підмодулів та оновить їх.
