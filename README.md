...
>
>Уявімо, що нам потрібно переглянути зміни в репозиторії встановленого модуля. Для цього переходимо в його директорію і прописуємо ``` git fetch ```, а потім ``` git merge ``` або обійтися одним ``` git pull ```. А якщо сабмодулів багато, можна прописати ``` git submodule update --remote ```.
